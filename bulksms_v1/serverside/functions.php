        ...
        $rows=count($num_rows);
         ...
         if($rows>0)
         {
             ...
             while($rowcount<$rows){                
                 ...
                 $data=$rowdata['profileID'].":".$rowdata['msisdn'].":".$rowdata['firstName'].":"
                         .$rowdata['otherNames'].":".$rowdata['YOB'].":".$rowdata['gender'].":".$rowdata['regionID'];       
                 array_push($contactsdetails, $data); 
                 $rowcount++;
             }
         }
       return $contactsdetails;  
        ...
        $num_rows = $stmtSel->fetchAll(PDO::FETCH_ASSOC);
         ...
         if($rows>0)
         {
             ...
             while($rowcount<$rows){                
                 ...
                 //merger the two arrays to form one bigger array
                 if(count($data)>0){
                     
                   foreach($data as $value){
                        array_push($contactsdetails, $value); 
                   }
                 }
                 $rowcount++;
             }
...
function GetMyAccountBalance(){  
        ...
        $clientID=$_SESSION['fields'][3];
        $stmtSel = $this->db->pdo->prepare("SELECT `availablebalance`, `actualbalance` FROM `tbaccount` WHERE `clientID`=:clientID");      
        $stmtSel->bindValue(':clientID', $clientID, PDO::PARAM_INT);
        $stmtSel->execute();
        $num_rows = $stmtSel->fetchAll(PDO::FETCH_ASSOC);
        $rows=count($num_rows);
         ...
         if($rows>0)
         {
             ...
             while($rowcount<$rows){                
                 $rowdata=$num_rows[$rowcount];
                 $data=$rowdata['availablebalance'].":".$rowdata['actualbalance'];       
                 array_push($accountbalances, $data); 
                 $rowcount++;
             }
         }
         else{
