...
class CWebApplication extends CApplication {
    ...
    public function runController($route) {
        ...
        $session=Yii::app()->session['validuserrights'];
       if (!empty(Yii::app()->session['validuserrights']) && !empty($route)) {
        if(($explode[1]!="logout")&&($explode[1]!="login")){
            if (($session == "MAERSK") && (($explode[0] != "MAERSKPayment") && ($explode[0] != "IB")&&($explode[0] != "Report")&&($explode[0] != "fundTransfer"))) {
                $route = "SecurityRights/SecurityRights";
            } else if (($session == "NATIONAL SECURITE") && (($explode[0] != "NSPayment") && ($explode[0] != "IB")&&($explode[0] != "Report")&&($explode[0] != "fundTransfer"))) {
                $route = "SecurityRights/SecurityRights";
            } else if (($session == "DJTELECOM") && (($explode[0] != "DJTELECOMPayment") && ($explode[0] != "IB")&&($explode[0] != "Report")&&($explode[0] != "fundTransfer"))) {
                $route = "SecurityRights/SecurityRights";
            }
        }
